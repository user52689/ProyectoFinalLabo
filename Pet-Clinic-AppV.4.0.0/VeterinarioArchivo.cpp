...
bool VeterinarioArchivo::guardar(int index, Veterinario veterinario) {
    ...
    fseek(_pFile, index * sizeof(Veterinario), SEEK_SET); //mueve el puntero al indice especificado

    grabo = fwrite(&veterinario, sizeof(Veterinario), 1, _pFile); // Escribe el veterinario en la posicion del index
...
int VeterinarioArchivo::buscar(int legajo) {
    ...
    while (fread(&veterinario, sizeof(Veterinario), 1, _pFile)) {
        if (veterinario.getLegajo() == legajo) {
            cerrar();
            return index; // Se encontro el veterinario, devuelve el indice
        }
        index++;
    }
...
bool VeterinarioArchivo::eliminar(int legajo) {
    ...
    if (index != -1) {
        ...
        veterinario.setEstado(false); // Cambia el estado
        return guardar(index, veterinario);
    }
    return false; // El veterinario no existr
}
